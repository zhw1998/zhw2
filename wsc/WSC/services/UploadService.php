...
class UploadService {
    ...
    public function __construct(){
        /* 获取配置 */
        $this->config   =   array_merge($this->config, C('upload'));
    ...
    private function setDriver(){
        require 'LocalUploadService.php';
        $this->uploader = new LocalUploadService();
    }
    ...
    private function check($file) {
        ...
        if (!$this->checkSize($file['size'])) {
            $this->error = '上传的图片不能超过'.round($this->maxSize/1024/1024).'M大小';
            return false;
        }
