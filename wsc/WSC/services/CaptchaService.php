...
class CaptchaService {
    ...
    public function __construct(){
        $this->config   =   array_merge($this->config, C('captcha'));
    }
    ...
    public function check($code, $id = '') {
        $key = $this->authcode($this->cypher).$id;
        // 验证码不能为空
        $secode = session($key);
    ...
    public function entry($id = '') {
        ...
        // 验证码使用随机字体
        $ttfPath = dirname(__FILE__) . '/../assets/fonts/captcha/';
    ...
    private function authcode($str){
        $key = substr(md5($this->cypher), 5, 8);
